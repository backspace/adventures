...
Upload the resulting `build/ios/ipa/landgrab.ipa` via Xcode → Window → Organizer, or `xcrun altool --upload-app`. Add to TestFlight Internal group.
...
- **Android**: generate a keystore once and keep it safe:
  ```bash
  keytool -genkey -v -keystore ~/keystores/landgrab-upload.jks \
  ...
  keyPassword=...
  keyAlias=upload
  storeFile=/Users/you/keystores/landgrab-upload.jks
  ```
