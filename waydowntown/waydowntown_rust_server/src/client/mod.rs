    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
    ...
    async fn answers_post(
        &self,
        param_answer_input: models::AnswerInput,
        context: &C) -> Result<AnswersPostResponse, ApiError>
    {
        ...
        // Body parameter
        let body = serde_json::to_string(&param_answer_input).expect("impossible to fail to serialize");
        ...
        match response.status().as_u16() {
            201 => {
                ...
                Ok(AnswersPostResponse::SuccessfullyCreatedAnswer
                    (body)
                )
            }
            code => {
        ...
        context: &C) -> Result<GamesIdGetResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::GamesIdGet200Response>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(GamesIdGetResponse::SuccessfullyFetchedGame
                    (body)
                )
            }
            code => {
    ...
    async fn games_post(
        &self,
        param_game_input: models::GameInput,
        param_concept: Option<String>,
        param_incarnation_filter_left_square_bracket_concept_right_square_bracket: Option<String>,
        context: &C) -> Result<GamesPostResponse, ApiError>
    {
        ...
        let query_string = {
            let mut query_string = form_urlencoded::Serializer::new("".to_owned());
            if let Some(param_concept) = param_concept {
                query_string.append_pair("concept",
                    &param_concept);
            }
            if let Some(param_incarnation_filter_left_square_bracket_concept_right_square_bracket) = param_incarnation_filter_left_square_bracket_concept_right_square_bracket {
                query_string.append_pair("incarnation_filter[concept]",
        ...
        let body = serde_json::to_string(&param_game_input).expect("impossible to fail to serialize");
        ...
        match response.status().as_u16() {
            201 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::GamesPost201Response>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(GamesPostResponse::SuccessfullyCreatedGame
                    (body)
                )
            }
            code => {
