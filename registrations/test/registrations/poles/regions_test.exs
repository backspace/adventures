defmodule Registrations.Poles.RegionsTest do
  ...
  describe "ancestor_chain/1" do
    ...
    test "returns root → self for a multi-level chain" do
      ...
      names = Regions.ancestor_chain(c) |> Enum.map(& &1.name)
    ...
    test "accepts a region_id string" do
      ...
      names = Regions.ancestor_chain(b.id) |> Enum.map(& &1.name)
  ...
  describe "search_regions/2" do
    test "matches case-insensitive substring" do
      ...
      names = Regions.search_regions("main") |> Enum.map(& &1.name)
