defmodule Registrations.Landgrab.TestPlayVisibilityTest do
  ...
  describe "Scope.test/2 with visibility_user_id" do
    ...
    test "validator sees only poles they created or are assigned (directly or via puzzlet)",
      ...
      alice_pole_ids =
        alice_scope |> Landgrab.list_poles_with_state() |> MapSet.new(& &1.pole.id)

      bob_pole_ids =
        bob_scope |> Landgrab.list_poles_with_state() |> MapSet.new(& &1.pole.id)
    ...
    test "unrestricted scope (supervisor/admin in test) sees all poles", %{session: session} do
      ...
      ids = unrestricted |> Landgrab.list_poles_with_state() |> MapSet.new(& &1.pole.id)
    ...
    test "scanning an invisible pole returns :not_found, not its existence",
      ...
      assert {:error, :not_found} =
               Landgrab.scan_payload(c_pole.barcode, nil, alice.id, scope)
    end
    ...
    test "scanning a visible pole works in restricted scope",
      ...
      assert {:ok, payload} = Landgrab.scan_payload(a_pole.barcode, nil, alice.id, scope)
    ...
    test "in test scope, active_puzzlet picks from visible puzzlets regardless of pole linkage",
         %{alice: alice, bob: bob, a_pole: a_pole, b_pole: b_pole, a_puzzlet: a_puzzlet, b_puzzlet: b_puzzlet} do
      ...
      assert Landgrab.active_puzzlet_for_pole(a_pole, alice.id, alice_scope).id == a_puzzlet.id
      ...
      assert Landgrab.active_puzzlet_for_pole(b_pole, bob.id, bob_scope).id == b_puzzlet.id
      assert Landgrab.active_puzzlet_for_pole(a_pole, bob.id, bob_scope).id == b_puzzlet.id
    end

    test "test scope picks from the 5 closest puzzlets (proximity), randomizing among them",
         %{author: author} do
      pole =
        insert(:pole, creator: author, status: :validated, barcode: "BC-prox#{u()}", latitude: 50.0, longitude: -100.0)
      ...
      close_puzzlets =
        for i <- 1..5 do
          insert(:puzzlet, creator: author, status: :validated, latitude: 50.0 + i * 0.001, longitude: -100.0)
        end

      far_a = insert(:puzzlet, creator: author, status: :validated, latitude: 60.0, longitude: -100.0)

      far_b = insert(:puzzlet, creator: author, status: :validated, latitude: 50.0, longitude: -120.0)

      session = insert_test_session(author)
      scope = Scope.test(session.id)

      close_ids = MapSet.new(close_puzzlets, & &1.id)

      # Run a bunch of rolls; only close puzzlets should ever come back.
      for_result =
        for _ <- 1..30 do
          Landgrab.active_puzzlet_for_pole(pole, nil, scope).id
        end
    ...
    test "real scope is never restricted by visibility_user_id (sanity)", %{c_pole: c_pole} do
      ...
      assert {:ok, _} = Landgrab.scan_payload(c_pole.barcode, nil, nil, Scope.real())
    end
  end
  ...
  defp insert_test_session(user) do
    {:ok, session} = Landgrab.create_test_session(user, %{name: "test"})
    session
  end
end
