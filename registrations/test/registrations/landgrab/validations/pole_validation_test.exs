defmodule Registrations.Landgrab.Validations.PoleValidationTest do
  ...
  describe "Lifecycle.validate_status_transition" do
    ...
    test "validator cannot accept" do
      ...
      refute changeset.valid?
      assert Enum.any?(errors_on(changeset).status, &String.contains?(&1, "cannot transition"))
    end
  ...
  describe "PoleValidationComment.changeset" do
    ...
    test "requires comment or suggested_value" do
      ...
      assert Enum.any?(errors_on(changeset).comment, &String.contains?(&1, "required"))
    end
