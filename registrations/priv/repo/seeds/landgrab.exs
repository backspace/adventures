# Seed data for the landgrab adventure. Idempotent: re-running won't duplicate.
#
#   ADVENTURE=landgrab mix run priv/repo/seeds/landgrab.exs
...
defmodule Seed do
  ...
  def upsert_pole(attrs, puzzlets) do
    case Repo.get_by(Registrations.Landgrab.Pole, barcode: attrs.barcode) do
      nil ->
        {:ok, pole} = Landgrab.create_pole(attrs)
  ...
  def upsert_draft_puzzlet(attrs, creator) do
    ...
    case existing do
      nil ->
        ...
        {:ok, _} = Landgrab.create_puzzlet(attrs)
...
poles = [
  %{
    ...
    puzzlets: [
      %{
        instructions: "Two rivers meet here. Name the one that flows in from the west.",
        answer: "Assiniboine",
        difficulty: 1
      },
  ...
  %{
    ...
    puzzlets: [
      ...
      %{
        instructions: "What neighbourhood does the bridge connect to on the east bank?",
        answer: "St. Boniface",
        difficulty: 4
      }
    ]
