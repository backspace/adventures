defmodule Registrations.Landgrab.Validations.PuzzletValidationComment do
  @moduledoc false
  use Ecto.Schema
  ...
  schema "puzzlet_validation_comments" do
    ...
    belongs_to(:puzzlet_validation, PuzzletValidation, type: :binary_id)
  ...
  def changeset(comment, attrs) do
    comment
    |> cast(attrs, [:puzzlet_validation_id, :field, :comment, :suggested_value, :status])
    |> validate_required([:puzzlet_validation_id, :field])
    |> validate_inclusion(:field, @valid_fields)
    |> validate_inclusion(:status, @valid_statuses)
    |> validate_has_content()
    |> assoc_constraint(:puzzlet_validation)
  end
