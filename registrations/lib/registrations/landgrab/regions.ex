defmodule Registrations.Landgrab.Regions do
  @moduledoc """
  ...
  defp reject_ancestor_cycle(changeset) do
    case {Ecto.Changeset.get_field(changeset, :id), Ecto.Changeset.get_change(changeset, :parent_region_id)} do
      {nil, _} ->
        changeset
  ...
  def inherited(region_id) when is_binary(region_id), do: inherited(ancestor_chain(region_id))
  ...
  defp region_summary(chain) when is_list(chain) do
    ...
    breadcrumb = Enum.map_join(chain, " > ", & &1.name)
