defmodule Registrations.Landgrab.Validations do
  @moduledoc """
  ...
  Status transitions are gated by `Registrations.Landgrab.Validations.Lifecycle`.
  """
  import Ecto.Query, warn: false

  alias Registrations.Landgrab.Pole
  alias Registrations.Landgrab.Puzzlet
  alias Registrations.Landgrab.Validations.Lifecycle
  ...
  # association inherits the parent's `landgrab` prefix and Ecto tries to
  # read from `landgrab.users` (which doesn't exist).
  ...
  defp unassignable?(%{status: "assigned"} = v) do
    case v do
      %PoleValidation{} ->
        not Repo.exists?(
          from(c in Registrations.Landgrab.Validations.PoleValidationComment,
      ...
      %PuzzletValidation{} ->
        not Repo.exists?(
          from(c in Registrations.Landgrab.Validations.PuzzletValidationComment,
  ...
  def get_pole_validation(id), do: PoleValidation |> Repo.get(id) |> preload_pole_validation()
  def get_puzzlet_validation(id), do: PuzzletValidation |> Repo.get(id) |> preload_puzzlet_validation()
  ...
  defp do_validator_transition(validation, validator_id, new_status, changeset_fun) do
    if validation.validator_id == validator_id do
      changeset =
        validation
  ...
  defp do_add_comment(validation, validator_id, attrs, schema, fk) do
    cond do
      ...
      true ->
        attrs = Map.put(attrs, Atom.to_string(fk), validation.id)
