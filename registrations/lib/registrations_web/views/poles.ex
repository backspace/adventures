defmodule RegistrationsWeb.Poles do
  @moduledoc """
  Compile-time loader for the OSM-derived river path data used by the
  LANDGRAB placeholder hero. The JSON is produced by
  `mix poles.fetch_rivers` and committed to the repo, so this module
  embeds the bytes at compile time and recompiles when the file on
  disk changes.

  Module/file naming follows the internal adventure key, `poles`. The
  public-facing brand is LANDGRAB; that name only appears in copy and
  CSS class hooks.
  """

  @rivers_path Path.join([:code.priv_dir(:registrations), "poles", "rivers.json"])
  ...
  @doc "Decoded rivers payload — see priv/poles/rivers.json"
  def data, do: @rivers
