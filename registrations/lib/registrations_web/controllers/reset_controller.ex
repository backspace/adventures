defmodule RegistrationsWeb.ResetController do
  ...
  def create(conn, %{"user" => user_params}) do
    user = Repo.get_by(RegistrationsWeb.User, email: user_params["email"])

    case RegistrationsWeb.Reset.create(user, Repo) do
      {:ok, user} ->
        Registrations.Mailer.send_password_reset(user)
  ...
  def edit(conn, %{"token" => token}) do
    case Repo.get_by(RegistrationsWeb.User, recovery_hash: token) do
      nil ->
        conn
  ...
  def update(conn, %{"user" => user_params}) do
    user =
      Repo.get_by(RegistrationsWeb.User, recovery_hash: user_params["recovery_hash"]) ||
        %User{}

    changeset = User.perform_reset_changeset(user, user_params)

    case RegistrationsWeb.Reset.update(changeset, Repo) do
      {:ok, user} ->
        conn
      ...
      {:error, changeset} ->
        ...
        changeset =
          Ecto.Changeset.put_change(changeset, :recovery_hash, user_params["recovery_hash"])
