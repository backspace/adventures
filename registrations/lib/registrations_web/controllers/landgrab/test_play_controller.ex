defmodule RegistrationsWeb.Landgrab.TestPlayController do
  @moduledoc """
  ...
  role via the `:landgrab_tester` pipeline.
  """
  ...
  def create_session(conn, params) do
    ...
    case Landgrab.create_test_session(user, %{name: params["name"]}) do
      {:ok, session} ->
  ...
  def list_sessions(conn, _params) do
    user = Pow.Plug.current_user(conn)
    sessions = Landgrab.list_test_sessions_for_user(user)
  ...
  def end_session(conn, %{"id" => id}) do
    ...
    case Landgrab.get_test_session(id) do
      nil ->
        not_found(conn)
      ...
      session ->
        {:ok, ended} = Landgrab.end_test_session(session)
  ...
  def list_poles(conn, %{"session_id" => session_id}) do
    case authorize_session(conn, session_id) do
      {:ok, _session} ->
        ...
        states = Landgrab.list_poles_with_state(scope)
  ...
  defp do_scan_pole(conn, user, scope, barcode) do
    case Landgrab.scan_payload(barcode, user.team_id, user.id, scope) do
      {:ok, state} ->
        json(conn, %{
      ...
      {:error, :already_owner, pole} ->
        ...
        |> json(%{
          ...
          pole: render_pole_state(Landgrab.pole_with_state(pole, scope))
        })
        ...
        case Landgrab.record_attempt(puzzlet, nil, user.id, answer, scope) do
            ...
            json(conn, %{
              ...
              pole:
                if pole do
                  %{
                    id: pole.id,
                    locked: Landgrab.pole_locked?(pole, scope),
          ...
          {:ok, %{result: :incorrect, attempts_remaining: remaining}} ->
            json(conn, %{
              ...
              previous_wrong_answers: Landgrab.team_wrong_answers(puzzlet, nil, scope)
            })
  ...
  defp authorize_session(conn, session_id) do
    ...
    case Landgrab.get_test_session(session_id) do
      nil ->
        {:halt, not_found(conn)}
