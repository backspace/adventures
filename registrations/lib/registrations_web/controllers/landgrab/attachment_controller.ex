defmodule RegistrationsWeb.Landgrab.AttachmentController do
  ...
  def show(conn, %{"id" => id}) do
    case Landgrab.get_attachment(id) do
      nil ->
  ...
  def delete(conn, %{"id" => id}) do
    ...
    case Landgrab.get_attachment(id) do
      ...
      %{creator_id: creator_id} = attachment when creator_id == user.id ->
        {:ok, _} = Landgrab.delete_attachment(attachment)
  ...
  defp insert_and_render(conn, attrs) do
    case Landgrab.create_attachment(attrs) do
      {:ok, attachment} ->
        conn
        |> put_status(:created)
