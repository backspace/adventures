defmodule Cr2016siteWeb.TeamController do
  ...
  def build(conn, %{"user_id" => base_user_id}) do
    ...
    relationships = Cr2016siteWeb.TeamFinder.relationships(base_user, users)
    ...
    changeset = Team.changeset(%Team{}, %{
      ...
      "notes" => team_users
        |> Enum.filter_map(fn(u) -> String.strip(u.accessibility || "") != "" end, &("#{String.split(&1.email, "@") |> hd}: #{&1.accessibility}"))
        |> Enum.join(", ")
    })
